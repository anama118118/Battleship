    ...
    func setupCards(){
        ...
        for m in 0..<randomNum.count { //0...4 in this case
            if randomBool(){ //Randomly call true or false
                if randomNum[m] < (100 - (10 * (arrayOfArrayOfShips[m].count - 1))) { //To make sure the largest element of the corresponding ship array is smaller than or equal to 100 (Vertically)
                    var arrayOfInt1: [Int] = []
                    for i in 0..<arrayOfArrayOfShips[m].count {
                        arrayOfInt1.append(randomNum[m] + 10 * i)
                        doubleCheckNoDuplicates.append(randomNum[m] + 10 * i)
                    }
                    arrayOfArrayOfShips[m] = arrayOfInt1 //ship laying in the vertical direction
                } else if (randomNum[m] / 10) == ((randomNum[m] + (arrayOfArrayOfShips[m].count - 1)) / 10) { //To make sure the largest element of the corresponding ship array is smaller than or equal to 100 (Horizontally). In case the above previous case fails.
                    var arrayOfInt2: [Int] = []
                    for i in 0..<arrayOfArrayOfShips[m].count {
                ...
                if (randomNum[m] / 10) == ((randomNum[m] + (arrayOfArrayOfShips[m].count - 1)) / 10) { //To make sure the largest element of the corresponding ship array is smaller than or equal to 100 (Horizontally)
                    var arrayOfInt4: [Int] = []
                    for i in 0..<arrayOfArrayOfShips[m].count {
                ...
                } else if randomNum[m] < (100 - 10 * (arrayOfArrayOfShips[m].count - 1)) { //To make sure the largest element of the corresponding ship array is smaller than or equal to 100 (Vertically). In case the above previous case fails.
                    var arrayOfInt5: [Int] = []
                    for i in 0..<arrayOfArrayOfShips[m].count {
        ...
        var doubleCheckDictionary : [Int: Int] = [:] //Check the presence of duplication. Resetup cards if present.
